...
{
    volatile unsigned char *lsr=(volatile unsigned char *)(UART_BASE_ADDR+0x05);
    volatile unsigned char *thr=(volatile unsigned char *)(UART_BASE_ADDR);
    if(c=='\n'){
        uart_send_char('\r');
    }
    while((*lsr&0x20)==0);
    *thr=c;
    //"You must implement the method uart_send_char to use this file!\n";
